...
The `Dockerfile.local` image honours the `QUX_HTTP_PORT` environment variable (default `8082`).
When used together with the supplied `docker-compose.yaml`, code changes are hot-reloaded via the
Compose watch configuration and the dev server remains reachable on the configured port:

```bash
# create a .env with (optional) overrides, e.g.
echo "QUX_HTTP_PORT=8082" > .env

# launch the container with filesystem watching and hot reload
